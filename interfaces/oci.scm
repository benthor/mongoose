    ...
    (if stmt
      (let* ((numcol (length sizes))
        ...
        (for-each
          (lambda (s a v i r)
            (set! position (+ position 1))
            (if (positive? (oci-bind-by-pos stmt "    " position v s TYPE_VARCHAR i a r OCI_IOV))
              (begin
                (cleanup)
                (error (error-message)))))
...
(define (pretty results)
  (if results
    (for-each
      (lambda (x) (write x) (newline))
      results)))

(define (all_tables svc owner)
  (select svc
    (string-append
...
(define (table_name->all_tables svc table_name)
  (select svc
    (string-append
      "select owner, table_name
       from all_tables
...
(define (all_tab_columns svc owner table)
  (select svc
    (string-append
...
(define (all-tables svc owner)
  (pretty (all_tables svc owner)))

(define (all-tab-columns svc owner table)
  (pretty (all_tab_columns svc owner table)))
